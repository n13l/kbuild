...
bgets_bb(struct fastbuf *f, struct bb_t *bb, uint limit)
{
  ...
  bb_grow(bb, 1);
  byte *buf = bb->ptr;
  uint len = 0, buf_len = _min(bb->len, limit);
  do
    {
      uint cnt = _min(src_len, buf_len);
      for (uint i = cnt; i--;)
        {
	  byte v = *src++;
	  if (v == '\n')
