...
interpret_set_dynamic(struct cf_item *item, int number, char **pars, void **ptr)
{
  ...
  *ptr = gary_init(size, number, &mem_std);
  return cf_parse_ary(number, pars, *ptr, type, &item->u);
}

static char *
interpret_add_dynamic(struct cf_item *item, int number, char **pars, 
                      int *processed, void **ptr, enum cf_operation op)
{
  enum cf_type type = item->type;
  void *old_p = *ptr;
  ...
  int taken = _min(number, abs(item->number)-old_nr);
  *processed = taken;
  // stretch the dynamic array
  void *new_p = gary_init(size, old_nr + taken, &mem_std);
  cf_journal_block(ptr, sizeof(void*));
  *ptr = new_p;
  if (op == OP_APPEND) {
...
static void
add_to_list(struct node *where, struct node *new_node, enum cf_operation op)
{
  switch (op)
  {
    ...
    case OP_COPY:
      ...
      list_insert_after(new_node, where);
      break;
    ...
    case OP_SET:
      ...
      list_insert_before(new_node, where);
      break;
    default:
      assert(0);
  }
...
interpret_add_bitmap(struct cf_item *item, int number, char **pars, int *processed, u32 *ptr, enum cf_operation op)
{
  ...
  for (int i=0; i<number; i++) {
    uint idx;
    if (item->type == CT_INT)
      TRY( cf_parse_int(pars[i], (int *)&idx) );
    else
      TRY( cf_parse_lookup(pars[i], (int *)&idx, item->u.lookup) );
    if (idx >= 32)
...
interpret_set_item(struct cf_item *item, int number, char **pars, int *processed, void *ptr, uint allow_dynamic)
{
  ...
  switch (item->cls)
  {
	...
	return "Dynamic array cannot be used here";
      taken = _min(number, abs(item->number));
      *processed = taken;
...
interpret_set_all(struct cf_item *item, void *ptr, enum cf_operation op)
{
  ...
  if (item->cls == CC_LIST) {
    cf_journal_block(ptr, sizeof(struct list));
    list_init(ptr);
    ...
    * (void**) ptr = GARY_FOREVER_EMPTY;
    ...
    memset(ptr, 0, item->number * sizeof(char*));
  } else
...
static void *
find_list_node(struct list *list, void *query, struct cf_section *sec, u32 mask)
{
  list_for_each(struct node *, n, *list)
  {
    uint found = 1;
    for (uint i=0; i<32; i++)
      if (mask & (1<<i))
...
cf_find_item(const char *name, struct cf_item *item)
{
  ...
  if (ci) {
    *item = *ci;
    ...
    memset(item, 0, sizeof(struct cf_item));
  return NULL;
}
