...
struct cf_parser_state {
  ...
  split_t word_buf;
  unsigned int words;
  unsigned int ends_by_brace;		// the line is ended by "{"
  bb_t copy_buf;
  unsigned int copied;
  char line_buf[];
};
...
get_word(struct cf_parser_state *p, uint is_command_name)
{
  ...
  if (*line == '\'') {
    ...
    char *tmp = stk_strunesc(p->copy_buf.ptr + start_copy);
    uint l = strlen(tmp);
    bb_grow(&p->copy_buf, start_copy + l + 1);
    strcpy((char *)(p->copy_buf.ptr + start_copy), tmp);
  ...
  *err = NULL;
  while (1) {
	...
	return NULL;
      ...
      if (!*p->line || *p->line == '#')
	sys_dbg("The line %s:%d following a backslash is empty", p->name_parse_fb ? : "", p->line_num);
    } else {
      split_grow(&p->word_buf, p->words+1);
      uint start = p->copied;
      p->word_buf.ptr[p->words++] = p->copied;
      *err = get_word(p, is_command_name);
      return *err ? NULL : (char *)(p->copy_buf.ptr + start);
    }
  }
}
...
parse_fastbuf(struct cf_context *cc, const char *name_fb, struct fastbuf *fb, uint depth)
{
  ...
  *p->line = 0;
  ...
  while (1)
  {
    ...
    if (!p->words)
      break;
    char *name = (char *)(p->copy_buf.ptr + p->word_buf.ptr[0]);
    char *pars[p->words-1];
    for (uint i=1; i<p->words; i++)
      pars[i-1] = (char *)(p->copy_buf.ptr + p->word_buf.ptr[i]);
	...
	if (optional_include && errno == ENOENT)
	  continue;
	err = cf_printf("Cannot open file %s", pars[0]);
	goto error;
      }
      uint ll = p->line_num;
	...
	case 'c': op = tolower((int)c[1]) == 'l' ? OP_CLEAR: OP_COPY; break;
	case 'a': switch (tolower((int)c[1])) {
		    case 'p': op = OP_APPEND; break;
		    case 'f': op = OP_AFTER; break;
		    default: op = OP_ALL;
		  }; break;
	case 'p': op = OP_PREPEND; break;
	case 'r': op = (c[1] && tolower((int)c[2]) == 'm') ? OP_REMOVE : OP_RESET; break;
...
error:
  if (name_fb)
    sys_dbg("File %s, line %d: %s", name_fb, p->line_num, err);
  else if (p->line_num == 1)
    sys_dbg("Manual setting of configuration: %s", err);
  else
    sys_dbg("Manual setting of configuration, line %d: %s", p->line_num, err);
...
load_file(struct cf_context *cc, const char *file)
{
  ...
  if (!fb) {
    sys_dbg("Cannot open configuration file %s:", file);
    return 1;
  }
...
cf_remember_entry(struct cf_context *cc, uint type, const char *arg)
{
  ...
  list_add_tail(&cc->conf_entries, &ce->n);
}
...
cf_reload(const char *file)
{
  ...
  struct list old_entries;
  list_move(&old_entries, &cc->conf_entries);
  cf_open_group();
  ...
  if (file)
    ...
    list_for_each(struct conf_entry *, ce, old_entries) {
      if (ce->type == CE_FILE)
  ...
  if (!err) {
    ...
    cf_journal_swap();
    list_move(&cc->conf_entries, &old_entries);
  }
  return err;
}
