...
void trans_throw(const char *id, void *object, const char *fmt, ...) _format_check(printf,3,4) _noreturn;

/** A `va_list` variant of @trans_throw(). **/
void trans_vthrow(const char *id, void *object, const char *fmt, va_list args) _noreturn;

/** Throw an already constructed exception (or re-throw an exception you have caught). **/
void trans_throw_exc(struct exception *x) _noreturn;
