...
struct fastbuf {
  ...
  off_t pos;				/* Position of bstop in the file */
  uint flags;					/* See enum fb_flags */
  int (*refill)(struct fastbuf *);		/* Get a buffer with new data, returns 0 on EOF */
  void (*spout)(struct fastbuf *);		/* Write buffer data to the file */
  int (*seek)(struct fastbuf *, off_t, int);/* Slow path for @bseek(), buffer already flushed; returns success */
...
static inline uint
fbbuf_count_written(struct fastbuf *f) /** Calculates, how many bytes were already written into the buffer. **/
{
 ...
 **/
uint
fbgrow_get_buf(struct fastbuf *b, byte **buf);
...
void bthrow(struct fastbuf *f, const char *id, const char *fmt, ...) _format_check(printf,3,4) _noreturn;	/** Throw exception on a given fastbuf **/
...
void bseek(struct fastbuf *f, off_t pos, int whence);	/** Seek in the buffer. See `man fseek` for description of @whence. Only for seekable fastbufs. **/
void bsetpos(struct fastbuf *f, off_t pos);			/** Set position to @pos bytes from beginning. Only for seekable fastbufs. **/
void brewind(struct fastbuf *f);				/** Go to the beginning of the fastbuf. Only for seekable ones. **/
off_t bfilesize(struct fastbuf *f);				/** How large is the file? -1 if not seekable. **/

static inline off_t btell(struct fastbuf *f)		/** Where am I (from the beginning)? **/
{
...
static inline uint
bavailr(struct fastbuf *f)			/** Return the length of the cached data to be read. Do not use directly. **/
{
  return f->bstop - f->bptr;
}

static inline uint
bavailw(struct fastbuf *f)			/** Return the length of the buffer available for writing. Do not use directly. **/
{
 ...
 */
static inline uint
bread(struct fastbuf *f, void *b, uint l)
{
  if (bavailr(f) >= l)
    {
      memcpy(b, f->bptr, l);
 ...
 */
static inline uint
breadb(struct fastbuf *f, void *b, uint l)
{
  if (bavailr(f) >= l)
    {
      memcpy(b, f->bptr, l);
 ...
 **/
uint
bgets_bb(struct fastbuf *f, struct bb_t *b, uint limit);
/**
