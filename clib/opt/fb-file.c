...
bfd_refill(struct fastbuf *f)
{
  ...
  if (F->wpos <= f->pos)
    {
      off_t diff = f->pos - F->wpos;
      /* Formula for long forward seeks (prefer lseek()) */
      if (diff > ((off_t)blen << 2))
        {
long_seek:
	  ...
	  while (skip)
	    {
	      int l = read(F->fd, f->buffer, _min(skip, blen));
	      if (unlikely(l <= 0)) {
		if (l < 0) {
		  bthrow(f, "read", "Error reading %s: ", f->name);
	        } else {
		    F->wpos -= skip;
		    goto eof;
		  }
...
seek:
      ...
      if (lseek(F->fd, F->wpos, SEEK_SET) < 0)
	bthrow(f, "read", "Error seeking %s: ", f->name);
    }
  /* Read (part of) buffer */
  do
    {
...
bfd_spout(struct fastbuf *f)
{
  ...
  if (FB_FILE(f)->wpos != f->pos && lseek(FB_FILE(f)->fd, f->pos, SEEK_SET) < 0)
    bthrow(f, "write", "Error seeking %s: ", f->name);
  ...
  while (l)
    {
      ...
      if (z <= 0)
	bthrow(f, "write", "Error writing %s: ", f->name);
      l -= z;
      c += z;
    }
...
static int
bfd_seek(struct fastbuf *f, off_t pos, int whence)
{
  ...
  switch (whence)
	...
	return 1;
      case SEEK_END: ;
	off_t l = lseek(FB_FILE(f)->fd, pos, SEEK_END);
	if (l < 0)
	  bthrow(f, "seek", "Error seeking %s: ", f->name);
	FB_FILE(f)->wpos = f->pos = l;
	FB_FILE(f)->wlen = 0;
	return 1;
      default:
