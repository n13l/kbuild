...
trans_open(void)
{
  ...
  c->current_trans = t;
  sys_dbg("Opened transaction %p", t);
  return t;
}
...
trans_commit(void)
{
  ...
  sys_dbg("Committing transaction %p", t);
  assert(t);
  assert(!t->thrown_exc);
  rp_commit(t->rpool);
  trans_pop(t, c);
...
trans_rollback(void)
{
  struct ucwlib_context *c = ucwlib_thread_context();
  struct trans *t = c->current_trans;
  sys_dbg("Rolling back transaction %p", t);
  assert(t);
  assert(!t->thrown_exc);
  rp_delete(t->rpool);
  trans_pop(t, c);
...
trans_fold(void)
{
  struct ucwlib_context *c = ucwlib_thread_context();
  struct trans *t = c->current_trans;
  sys_dbg("Folding transaction %p", t);
  assert(t);
  assert(!t->thrown_exc);
  trans_pop(t, c);
...
trans_throw_exc(struct exception *x)
{
  struct ucwlib_context *c = ucwlib_thread_context();
  struct trans *t = c->current_trans;
  sys_dbg("Throwing exception %s (%s) in trans %p", x->id, x->msg, t);
  if (!t)
    die("%s", x->msg);
  ...
  while (t->thrown_exc)
	...
	die("%s", x->msg);
      ...
      trans_fold();
      t = c->current_trans;
      sys_dbg("... recursive throw propagated to parent trans %p", t);
    }
