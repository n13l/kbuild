...
#ifndef ASORT_SWAP
#define ASORT_SWAP(i,j) \
	do { ASORT_KEY_TYPE tmp = ASORT_ELT(i); ASORT_ELT(i)=ASORT_ELT(j); \
		ASORT_ELT(j)=tmp; } while (0)
#endif
 ...
 **/
static void
ASORT_PREFIX(sort)(ASORT_ARRAY_ARG uint array_size ASORT_EXTRA_ARGS)
{
  ...
  r = _min(array_size, ASORT_THRESHOLD);
  m = 0;
  for (l=1; l<r; l++)
