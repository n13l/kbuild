 ...
 * to an empty state.
 **/
static void _unused GBUF_PREFIX(done)(BUF_T *b)
{
  if (b->ptr)
    xfree(b->ptr);
  b->ptr = NULL;
 ...
 * for growing.
 **/
static void _unused GBUF_PREFIX(set_size)(BUF_T *b, uint len)
{
  b->len = len;
...
static void _unused GBUF_PREFIX(do_grow)(BUF_T *b, uint len)
{
